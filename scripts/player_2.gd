...
func _physics_process(delta: float) -> void:
	...
	if direction != 0: # Player is pressing left or right
		...
		# --- Play Move Animation (only if on floor) ---
		if on_floor:
			animated_sprite.play("move")
	else: # Player is not pressing left or right
		velocity.x = move_toward(velocity.x, 0, SPEED)
		# --- Play Idle Animation (only if on floor) ---
		if on_floor:
